...
import { Building2, Percent, RefreshCcw, Save, Search, Wallet } from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-50/50 p-6 lg:p-8">
      <div className="mx-auto max-w-7xl">
        <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm font-medium text-slate-500">
              Each rental booking snapshots the selected store rules. Store share is applied first, owner share is applied on the remaining amount, and admin keeps the rest.
            </p>
          </div>
        ...
        <div className="mb-5 rounded-[28px] border border-amber-100 bg-amber-50 px-5 py-4 text-sm font-semibold text-amber-900">
          Preview math uses `Rs {COMMISSION_SAMPLE_AMOUNT}`. Example: if store is `10%` and owner is `20%`, the store gets `Rs 100`, owner gets `Rs 180`, and admin keeps `Rs 720`.
        </div>

        <div className="overflow-hidden rounded-[32px] border border-slate-100 bg-white shadow-sm">
          <div className="grid grid-cols-[minmax(0,1.2fr)_minmax(0,0.9fr)_minmax(0,0.9fr)_minmax(0,0.9fr)_150px] gap-4 bg-slate-100 px-6 py-5 text-sm font-black text-slate-700">
            <p>Service Store</p>
            ...
            <div className="divide-y divide-slate-100 bg-white">
              {filteredStores.map((store) => {
                ...
                return (
                  <div
                    key={storeId}
                    className="grid grid-cols-[minmax(0,1.2fr)_minmax(0,0.9fr)_minmax(0,0.9fr)_minmax(0,0.9fr)_150px] gap-4 px-6 py-5"
                  >
